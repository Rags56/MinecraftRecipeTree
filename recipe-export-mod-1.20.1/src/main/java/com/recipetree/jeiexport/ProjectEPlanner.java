...
final class ProjectEPlanner {
    ...
    static final ResourceLocation ID = new ResourceLocation("projecte", "emc");
    ...
    static final class Category implements IRecipeCategory<Recipe> {
        ...
        @Override public ResourceLocation getRegistryName(Recipe recipe) {
            return new ResourceLocation("projecte", "emc/" + Naming.hash8("item|" + JeiExportPlugin.runtime().getIngredientManager().getIngredientHelper(VanillaTypes.ITEM_STACK).getUniqueId(recipe.output, UidContext.Ingredient)));
        }
        ...
        @Override public void draw(Recipe recipe, IRecipeSlotsView slots, GuiGraphics graphics, double mouseX, double mouseY) {
            graphics.blit(new ResourceLocation("projecte", "textures/gui/transmute.png"), 0, 0, 8, 16, 80, 62);
